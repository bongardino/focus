class Api::CalendarsController < ApplicationController
  ...
  def google_secret
	...
	def get_events 	  # Initialize Google Calendar API
	  service = Google::Apis::CalendarV3::CalendarService.new
	  # Use google keys to authorize
	  service.authorization = google_secret.to_authorization
	  # Request for a new aceess token just incase it expired
	...
	def get_events_month 	  # Initialize Google Calendar API
	  service = Google::Apis::CalendarV3::CalendarService.new
	  # Use google keys to authorize
	  service.authorization = google_secret.to_authorization
	  # Request for a new aceess token just incase it expired
	  service.authorization.refresh!
		...
		response = service.list_events(
			...
			time_max: Time.now.advance(:days => +16).iso8601
			)
	...
	def build_month
		json_success = []
		get_events_month.each do |event|
			if !Event.find_by(uid: event.id).nil?																		# replace this with find or create by, to update existing events instead of skipping them entirely
				new_event = Event.find_by(uid: event.id)
			else
				new_event = Event.new(
